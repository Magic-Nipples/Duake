...
void PM_FixupGravityVelocity ()
{
	...
	// Get the correct velocity for the end of the dt 
  	pmove->velocity[2] -= (ent_gravity * pmove->movevars->gravity * pmove->frametime * 0.6 ); //0.5
...
void PM_Jump (void)
{
	...
	//pmove->PM_PlaySound(CHAN_BODY, "player/jump1.wav", 0.5, ATTN_NORM, 0, PITCH_NORM); //magic nipples - jump sound here works 100% over player.cpp

	if(pmove->RandomLong(0, 1) == 1)
		pmove->PM_PlaySound(CHAN_AUTO, "player/jump_ft1.wav", 1.0, ATTN_NORM, 0, PITCH_NORM);
	else
		pmove->PM_PlaySound(CHAN_AUTO, "player/jump_ft2.wav", 1.0, ATTN_NORM, 0, PITCH_NORM);

	pmove->velocity[2] += 285;//270;
...
void PM_CheckFalling( void )
{
		 ...
		 pmove->flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		...
		else
		{
			...
			pmove->vuser3[2] = pmove->flFallVelocity * -0.36;	// punch z axis

			if(pmove->vuser3[2] < -340)
				pmove->vuser3[2] = -340;
		}
	}		
...
void PM_PlayerMove ( qboolean server )
{
	...
	if (pmove->alwaysrun)
	{
		...
		{
			VectorScale(pmove->velocity, 0.95, pmove->velocity);
		}
	}
	else
	{
