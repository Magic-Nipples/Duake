...
const char* uiSoundOut			= "sound/misc/menu5.wav"; //"media/launch_dnmenu1.wav";
const char* uiSoundLaunch		= "sound/misc/menu2.wav";
const char* uiSoundGlow			= "sound/misc/menu3.wav";
const char* uiSoundBuzz			= "media/launch_deny2.wav";
const char* uiSoundKey			= "sound/misc/menu3.wav"; //key selection menu
const char* uiSoundRemoveKey	= "media/launch_deny1.wav";

const char* uiSoundHide			= "sound/misc/menu5.wav";
const char* uiSoundUnhide		= "sound/misc/menu4.wav";
...
int		uiColorDoomRed = 0xFFED0000;
int		uiColorDoomSelect = 0xFFff9b9b;
int		uiColorDoomSave = 0xFF0000e3;
...
=================
*/
void UI_StretchCoords(int* x, int* y, int* w, int* h)
{
	if (x) *x *= uiStatic.stretchX;
	if (y) *y *= uiStatic.stretchY;
	if (w) *w *= uiStatic.stretchX;
	if (h) *h *= uiStatic.stretchY;
}

void UI_ScaleCoords( int *x, int *y, int *w, int *h, boolean centered)
{
	float hWidthStart, hWidthEnd = 0;

	float xscale = ScreenWidth / uiStatic.scaleX;
	float yscale = ScreenHeight / uiStatic.scaleY;

	if(centered)
		hWidthStart = *w * 0.5;
	else
		if (x) *x *= xscale; //if (x) *x *= uiStatic.stretchX;
...
{
	HIMAGE hPic = PIC_Load( pic, TF_NEAREST | TF_NOMIPMAP);
...
void UI_DrawPicHoles(int x, int y, int width, int height, const int color, const char* pic)
{
	...
	PIC_Set(hPic, r, g, b);
	SPR_DrawHoles(x, y, width, height);
}
...
bool UI_LoadSteamBackground( void )
{
	...
	if( !pfile || _stricmp( token, "resolution" )) // resolution at first!
		goto cleanup;
...
void UI_AddItem( menuFramework_s *menu, void *item )
{
	...
	case QMTYPE_ACTION:
		...
		break;
	case QMTYPE_BITMAP:
		UI_Bitmap_Init((menuBitmap_s*)item, false);
		break;
	case QMTYPE_BITMAP_STRETCH:
		UI_Bitmap_Init((menuBitmap_s*)item, true);
		break;
...
void UI_AddServerToList( netadr_t adr, const char *info )
{
	...
	if( _stricmp( gMenu.m_gameinfo.gamefolder, Info_ValueForKey( info, "gamedir" )))
		return;

	// ignore if duplicated
	for( i = 0; i < uiStatic.numServers; i++ )
	{
		if( !_stricmp( uiStatic.serverNames[i], info ))
			return;
	}
...
int UI_VidInit( void )
{
	...
	//float xscale = fabs(4800000 / ScreenHeight) * 0.0001;
	float yscale = fabs(2400000 / ScreenHeight) * 0.0001;	
	uiStatic.scaleX = ScreenWidth * yscale;
	uiStatic.scaleY = ScreenHeight * yscale;

	uiStatic.stretchX = ScreenWidth / 1024.0f;
	uiStatic.stretchY = ScreenHeight / 768.0f;
	...
	UI_StretchCoords( NULL, NULL, &uiStatic.outlineWidth, NULL );
	UI_StretchCoords( NULL, NULL, &uiStatic.sliderWidth, NULL );
	UI_StretchCoords( NULL, NULL, &uiStatic.buttons_draw_width, &uiStatic.buttons_draw_height );
