...
{ 255, 255, 255, 255 },	// default color (can be changed by user)
};
...
static void Con_LoadConsoleFont( int fontNumber, cl_font_t *font )
{
	...
	//if( Sys_CheckParm( "-oldfont" ))
		Con_LoadVariableWidthFont( "gfx.wad/creditsfont.fnt", font ); //QUAKE - load old console font 
...
static void Con_LoadConchars( void )
{
	...
	float xscale = fabs(3200000 / glState.height) * 0.0001;
	float xsnail = glState.width * xscale;
	con.scale = glState.width / xsnail;

	// sets the current font
	con.lastUsedFont = con.curFont = &con.chars[1];
}
...
static void Con_AddCommandToList( const char *s, const char *unused1, const char *unused2, void *unused3 )
{
	...
	if( Q__strnicmp( s, con.completionString, Q_strlen( con.completionString )))
		return; // no match
...
{
	if( !Q__strnicmp( s, con.shortestMatch, Q_strlen( con.shortestMatch )))
	{
...
void Con_DrawInput( int lines )
{
	...
	Field_DrawInputLine( 16 * con.scale, y, &con.input ); //QUAKE - this is wider with quake font | 16
}
...
void Con_DrawVersion( void )
{
	...
	int	start, height = 4;// glState.height;
	...
	Q_snprintf(curbuild, MAX_STRING, "Duake Test Build(%i)", Q_buildnum());
	Con_DrawStringLen( curbuild, &stringLen, &charH );
...
void Con_VidInit( void )
{
	...
	if( !con.background ) // last chance - quake conback image
	{
		...
		if( !Q__stricmp( FS_Gamedir(), "id1" ))
			draw_to_console = true;

		if( !Q__stricmp( FS_Gamedir(), "hipnotic" ))
			draw_to_console = true;

		if( !Q__stricmp( FS_Gamedir(), "rogue" ))
			draw_to_console = true;
