...
void Sys_ParseCommandLine( LPSTR lpCmdLine, qboolean uncensored )
{
	...
	for( i = 0; i < host.argc; i++ )
	{
		...
		if( !Q__stricmp( "-game", host.argv[i] )) host.argv[i] = (char *)blank;
		// probably it's timewaster, because engine rejected second change
		if( !Q__stricmp( "+game", host.argv[i] )) host.argv[i] = (char *)blank;
		// you sure what is map exists in new game?
		if( !Q__stricmp( "+map", host.argv[i] )) host.argv[i] = (char *)blank;
		// just stupid action
		if( !Q__stricmp( "+load", host.argv[i] )) host.argv[i] = (char *)blank;
		// changelevel beetwen games? wow it's great idea!
		if( !Q__stricmp( "+changelevel", host.argv[i] )) host.argv[i] = (char *)blank;
	}
}
...
int Sys_CheckParm( const char *parm )
{
	...
	for( i = 1; i < host.argc; i++ )
	{
		...
		if( !Q__stricmp( parm, host.argv[i] ))
			return i;
	}
	return 0;
}
