...
static void stringlistappend( stringlist_t *list, char *text )
{
	...
	if( !Q__stricmp( text, "." ) || !Q__stricmp( text, ".." ))
...
static dpackfile_t *FS_AddFileToPack( const char *name, pack_t *pack, long offset, long size )
{
	...
	while( left <= right )
	{
		...
		middle = (left + right) / 2;
		diff = Q__stricmp( pack->files[middle].name, name );
...
pack_t *FS_LoadPackPAK( const char *packfile, int *error )
{
	...
	packhandle = _open( packfile, O_RDONLY|O_BINARY );
	...
	_read( packhandle, (void *)&header, sizeof( header ));
	...
	_lseek( packhandle, header.dirofs, SEEK_SET );

	if( header.dirlen != _read( packhandle, (void *)info, header.dirlen ))
	{
...
static qboolean FS_AddWad_Fullpath( const char *wadfile, qboolean *already_loaded, int flags )
{
	...
	{
		if( search->wad && !Q__stricmp( search->wad->filename, wadfile ))
		{
...
static qboolean FS_AddPak_Fullpath( const char *pakfile, qboolean *already_loaded, int flags )
{
	...
	for( search = fs_searchpaths; search; search = search->next )
	{
		if( search->pack && !Q__stricmp( search->pack->filename, pakfile ))
		{
			if( already_loaded ) *already_loaded = true;
			return true; // already loaded
		}
	...
	if( pak )
	{
		...
		{
			if( !Q__stricmp( COM_FileExtension( pak->files[i].name ), "wad" ))
			{
...
void FS_AddGameDirectory( const char *dir, int flags )
{
	...
	for( i = 0; i < list.numstrings; i++ )
	{
		if( !Q__stricmp( COM_FileExtension( list.strings[i] ), "pak" ))
		{
	...
	for( i = 0; i < list.numstrings; i++ )
	{
		if( !Q__stricmp( COM_FileExtension( list.strings[i] ), "wad" ))
		{
			Q_sprintf( fullpath, "%s%s", dir, list.strings[i] );
			FS_AddWad_Fullpath( fullpath, NULL, flags );
		}
...
void FS_Rescan( void )
{
	...
	if( Q__stricmp( GI->basedir, GI->gamedir ))
		FS_AddGameHierarchy( GI->basedir, 0 );
	if( Q__stricmp( GI->basedir, GI->falldir ) && Q__stricmp( GI->gamedir, GI->falldir ))
...
static qboolean FS_ParseLiblistGam( const char *filename, const char *gamedir, gameinfo_t *GameInfo )
{
	...
	while(( pfile = COM_ParseFile( pfile, token )) != NULL )
	{
		...
		else if( !Q__stricmp( token, "version" ))
		{
			pfile = COM_ParseFile( pfile, token );
			GameInfo->version = Q_atof( token );
		}
		else if( !Q__stricmp( token, "size" ))
		{
			pfile = COM_ParseFile( pfile, token );
			GameInfo->size = Q_atoi( token );
		}
		else if( !Q__stricmp( token, "edicts" ))
		{
			pfile = COM_ParseFile( pfile, token );
			GameInfo->max_edicts = Q_atoi( token );
		}
		else if( !Q__stricmp( token, "mpentity" ))
		{
			pfile = COM_ParseFile( pfile, GameInfo->mp_entity );
		}
		else if( !Q__stricmp( token, "mpfilter" ))
		{
			pfile = COM_ParseFile( pfile, GameInfo->mp_filter );
		}
		else if( !Q__stricmp( token, "secure" ))
		{
			pfile = COM_ParseFile( pfile, token );
			GameInfo->secure = Q_atoi( token );
		}
		else if( !Q__stricmp( token, "nomodels" ))
		{
			pfile = COM_ParseFile( pfile, token );
...
static qboolean FS_ReadGameInfo( const char *filepath, const char *gamedir, gameinfo_t *GameInfo )
{
	...
	while(( pfile = COM_ParseFile( pfile, token )) != NULL )
	{
		if( !Q__stricmp( token, "basedir" ))
		{
			pfile = COM_ParseFile( pfile, fs_path );
			if( Q__stricmp( fs_path, GameInfo->basedir ) || Q__stricmp( fs_path, GameInfo->gamedir ))
				Q_strncpy( GameInfo->basedir, fs_path, sizeof( GameInfo->basedir ));
		}
		else if( !Q__stricmp( token, "fallback_dir" ))
		{
			pfile = COM_ParseFile( pfile, fs_path );
			if( Q__stricmp( fs_path, GameInfo->basedir ) || Q__stricmp( fs_path, GameInfo->falldir ))
				Q_strncpy( GameInfo->falldir, fs_path, sizeof( GameInfo->falldir ));
		}
		else if( !Q__stricmp( token, "gamedir" ))
		{
			pfile = COM_ParseFile( pfile, fs_path );
			if( Q__stricmp( fs_path, GameInfo->basedir ) || Q__stricmp( fs_path, GameInfo->gamedir ))
				Q_strncpy( GameInfo->gamedir, fs_path, sizeof( GameInfo->gamedir ));
		}
		else if( !Q__stricmp( token, "title" ))
		{
			pfile = COM_ParseFile( pfile, GameInfo->title );
		}
		else if( !Q__stricmp( token, "sp_entity" ))
		{
			pfile = COM_ParseFile( pfile, GameInfo->sp_entity );
		}
		else if( !Q__stricmp( token, "mp_entity" ))
		{
			pfile = COM_ParseFile( pfile, GameInfo->mp_entity );
		}
		else if( !Q__stricmp( token, "mp_filter" ))
		{
			pfile = COM_ParseFile( pfile, GameInfo->mp_filter );
		}
		else if( !Q__stricmp( token, "gamedll" ))
		...
		else if( !Q__stricmp( token, "dllpath" ))
		{
		...
		else if( !Q__stricmp( token, "trainmap" ))
		{
			pfile = COM_ParseFile( pfile, GameInfo->trainmap );
			COM_StripExtension( GameInfo->trainmap ); // HQ2:Amen has extension .bsp
		}
		...
		else if( !Q__stricmp( token, "url_update" ))
		{
			pfile = COM_ParseFile( pfile, GameInfo->update_url );
		}
		else if( !Q__stricmp( token, "date" ))
		{
		...
		else if( !Q__stricmp( token, "noskills" ))
		{
			pfile = COM_ParseFile( pfile, token );
			GameInfo->noskills = Q_atoi( token );
		}
		else if( !Q__strnicmp( token, "ambient", 7 ))
		{
...
static qboolean FS_ParseGameInfo( const char *gamedir, gameinfo_t *GameInfo )
{
	...
	if(( FS_CheckForGameDir( gamedir ) || !Q__stricmp( fs_gamedir, gamedir )) && !FS_FileExists( gameinfo_path, false ))
	{
...
void FS_LoadGameInfo( const char *rootfolder )
{
	...
	{
		if( !Q__stricmp( SI.games[i]->gamefolder, fs_gamedir ))
			break;
	}
...
void FS_Init( void )
{
	...
	{
		if( !Q__stricmp( fs_basedir, dirs.strings[i] ))
			hasBaseDir = true;

		if( !Q__stricmp( fs_gamedir, dirs.strings[i] ))
			hasGameDir = true;
	}
	...
	for( i = 0; i < dirs.numstrings; i++ )
	{
		if( !FS_SysFolderExists( dirs.strings[i] ) || ( !Q__stricmp( dirs.strings[i], ".." ) && !fs_ext_path ))
			continue;
...
static file_t *FS_SysOpen( const char *filepath, const char *mode )
{
	...
	file->handle = _open( filepath, mod|opt, 0666 );
	...
	file->real_length = _lseek( file->handle, 0, SEEK_END );

	// For files opened in append mode, we start at the end of the file
	if( mod & O_APPEND ) file->position = file->real_length;
	else _lseek( file->handle, 0, SEEK_SET );
...
file_t *FS_OpenPackedFile( pack_t *pack, int pack_ind )
{
	...
	if( _lseek( pack->handle, pfile->filepos, SEEK_SET ) == -1 )
		return NULL;

	dup_handle = _dup( pack->handle );
...
qboolean FS_SysFileExists( const char *path )
{
	...
	if(( desc = _open( path, O_RDONLY|O_BINARY )) < 0 )
		return false;
	...
	for( search = fs_searchpaths; search; search = search->next )
	{
		...
		if( search->pack )
		{
			...
			while( left <= right )
			{
				...
				middle = (left + right) / 2;
				diff = Q__stricmp( pak->files[middle].name, name );
		...
		else if( search->wad )
		{
			...
			if( !anywadname && Q__stricmp( wadname, shortname ))
				continue;
...
long FS_Write( file_t *file, const void *data, size_t datasize )
{
	...
	if( file->buff_ind != file->buff_len )
		_lseek( file->handle, file->buff_ind - file->buff_len, SEEK_CUR );

	// purge cached data
	FS_Purge( file );

	// write the buffer and update the position
	result = _write( file->handle, data, (long)datasize );
	file->position = _lseek( file->handle, 0, SEEK_CUR );
...
long FS_Read( file_t *file, void *buffer, size_t buffersize )
{
	...
	if( buffersize > sizeof( file->buff ) / 2 )
	{
		...
		_lseek( file->handle, file->offset + file->position, SEEK_SET );
		nb = _read (file->handle, &((byte *)buffer)[done], count );
	...
	else
	{
		...
		_lseek( file->handle, file->offset + file->position, SEEK_SET );
		nb = _read( file->handle, file->buff, count );
...
int FS_VPrintf( file_t *file, const char *format, va_list ap )
{
	...
	len = _write( file->handle, tempbuff, len );
	Mem_Free( tempbuff );
...
int FS_Seek( file_t *file, long offset, int whence )
{
	...
	if( _lseek( file->handle, file->offset + offset, SEEK_SET ) == -1 )
		return -1;
...
dll_user_t *FS_FindLibrary( const char *dllname, qboolean directpath )
{
	...
	if( !Q__strnicmp( dllname, "..\\valve\\", 9 ) || !Q__strnicmp( dllname, "../valve/", 9 ))
		start += 9;
...
search_t *FS_Search( const char *pattern, int caseinsensitive, int gamedironly )
{
	...
	for( searchpath = fs_searchpaths; searchpath; searchpath = searchpath->next )
	{
		...
		else if( searchpath->wad )
		{
			...
			// quick reject by wadname
			if( !anywadname && Q__stricmp( wadname, temp2 ))
				continue;
...
static dlumpinfo_t *W_FindLump( wfile_t *wad, const char *name, const char matchtype )
{
	...
	while( left <= right )
	{
		...
		int	diff = Q__stricmp( wad->lumps[middle].name, name );
...
wfile_t *W_Open( const char *filename, int *error )
{
	...
	for( i = 0; i < lumpcount; i++ )
	{
		...
		if( srclumps[i].type == 68 && !Q__stricmp( srclumps[i].name, "conchars" ))
