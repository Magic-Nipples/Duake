...
void Host_ChangeGame_f( void )
{
	...
	{
		if( !Q__stricmp( SI.games[i]->gamefolder, Cmd_Argv( 1 )))
			break;
	}
	...
	else if( !Q__stricmp( GI->gamefolder, Cmd_Argv( 1 )))
	{
...
void Host_Exec_f( void )
{
	...
	if( !Q__stricmp( "game.cfg", Cmd_Argv( 1 )))
	{
	...
	if( !Q__stricmp( "config.cfg", Cmd_Argv( 1 )))
...
qboolean Host_RegisterDecal( const char *name, int *count )
{
	...
	{
		if( !Q__stricmp( host.draw_decals[i], shortname ))
			return true;
	}
...
void Host_InitCommon( const char *hostname, qboolean bChangeGame )
{
	...
	*out = '\0'; // write terminator
	...
	if( Sys_CheckParm( "-console" ) || !Q__stricmp( SI.exeName, "quake" ) || !Q__stricmp(SI.exeName, "console") || !Q__stricmp(SI.exeName, "developer")) //magic nipples - if console.exe allow console
		host.allow_console = true;

	if( Sys_CheckParm( "-dev" ) || !Q__stricmp(SI.exeName, "developer")) //magic nipples - if developer.exe allow dev stuff
	{
		host.allow_console = true;
	...
	if( Q__stricmp( host.rootdir, szRootPath ))
	{
