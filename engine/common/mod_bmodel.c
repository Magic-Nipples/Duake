...
static void Mod_FindModelOrigin( const char *entities, const char *modelname, vec3_t origin )
{
	...
	while(( pfile = COM_ParseFile( pfile, token )) != NULL )
	{
		...
		while( 1 )
		{
			...
			if( !Q__stricmp( keyname, "model" ) && !Q__stricmp( modelname, token ))
				model_found = true;

			if( !Q__stricmp( keyname, "origin" ))
			{
...
{
	if( !Q__strnicmp( name, "SKY", 3 ))
		return CONTENTS_SKY;

	if( name[0] == '!' || name[0] == '*' )
	{
		if( !Q__strnicmp( name + 1, "lava", 4 ))
			return CONTENTS_LAVA;
		else if( !Q__strnicmp( name + 1, "slime", 5 ))
			return CONTENTS_SLIME;
		return CONTENTS_WATER; // otherwise it's water
	}

	if( !Q__strnicmp( name, "water", 5 ))
...
static void Mod_SetupSubmodels( dbspmodel_t *bmod )
{
	...
	for( i = 0; i < mod->numsubmodels; i++ )
	{
		...
		if( i != 0 )
		{
			...
			if( !Q__stricmp( loadmodel->name, "maps/c2a1.bsp" ) && ( i == 11 ))
				SetBits( mod->flags, MODEL_HAS_ORIGIN );
#endif
		}
...
static void Mod_LoadEntities( dbspmodel_t *bmod )
{
	...
	while(( pfile = COM_ParseFile( pfile, token )) != NULL )
	{
		...
		while( 1 )
		{
			...
			else if( !Q__stricmp( keyname, "message" ))
				Q_strncpy( world.message, token, sizeof( world.message ));
			else if( !Q__stricmp( keyname, "compiler" ) || !Q__stricmp( keyname, "_compiler" ))
				Q_strncpy( world.compiler, token, sizeof( world.compiler ));
			else if( !Q__stricmp( keyname, "generator" ) || !Q__stricmp( keyname, "_generator" ))
...
{
	if ((name[0] == '*' && Q__stricmp(name, "*default")) || name[0] == '!')
		return true;

	if (!CL_IsQuakeCompatible())
	{
		if (!Q_strncmp(name, "water", 5) || !Q__strnicmp(name, "laser", 5))
			return true;
	}
...
static void Mod_LoadSurfaces( dbspmodel_t *bmod )
{
	...
	for( i = 0; i < bmod->numsurfaces; i++, out++, info++ )
	{
		...
		//if(( tex->name[0] == '*' && Q__stricmp( tex->name, "*default" )) || tex->name[0] == '!' )
		//	SetBits( out->flags, SURF_DRAWTURB );

		//if( !CL_IsQuakeCompatible( ))
		//{
		//	if( !Q_strncmp( tex->name, "water", 5 ) || !Q__strnicmp( tex->name, "laser", 5 ))
		//		SetBits( out->flags, SURF_DRAWTURB );
		//}
