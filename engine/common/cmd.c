...
{	
	...
	if( !host.apply_game_config )
	{
		...
		for( a = cmd_alias; a; a = a->next )
		{
			if( !Q__stricmp( cmd_argv[0], a->name ))
			{
	...
	if( !host.apply_game_config || !Q_strcmp( cmd_argv[0], "exec" ))
	{
		...
		for( cmd = cmd_functions; cmd; cmd = cmd->next )
		{
			if( !Q__stricmp( cmd_argv[0], cmd->name ) && cmd->function )
			{
				cmd->function();
				return;
			}
...
void Cmd_ForwardToServer( void )
{
	...
	if( cls.demoplayback )
	{
		if( !Q__stricmp( Cmd_Argv( 0 ), "pause" ))
			cl.paused ^= 1;
		return;
	}
	...
	{
		if( Q__stricmp( Cmd_Argv( 0 ), "setinfo" ))
...
void Cmd_List_f( void )
{
	...
	for( cmd = cmd_functions; cmd; cmd = cmd->next )
	{
		...
		if( match && !Q__stricmpext( match, cmd->name ))
			continue;
