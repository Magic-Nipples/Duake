...
void SV_ConnectClient( netadr_t from )
{
	...
	if( !NET_IsLocalAddress( from ) && sv_password.string[0] && Q__stricmp( sv_password.string, Info_ValueForKey( userinfo, "password" )))
	{
...
void SV_PutClientInServer( sv_client_t *cl )
{
	...
	if( FBitSet( ent->v.flags, FL_GODMODE|FL_NOTARGET ) && !Q__stricmp( GI->gamefolder, "invasion" ))
...
void SV_UserinfoChanged( sv_client_t *cl )
{
	...
	while( 1 )
	{
		for( i = 0, current = svs.clients; i < svs.maxclients; i++, current++ )
		{
			...
			if( !Q__stricmp( current->name, val ))
				break;
		}
...
static qboolean SV_DownloadFile_f( sv_client_t *cl )
{
	...
	if( name[0] != '!' )
	{
		if( sv_send_resources.value )
		{
			...
			if( !Q__stricmp( COM_FileExtension( name ), "mdl" ))
			{
	...
	if( Q_strlen( name ) == 36 && !Q__strnicmp( name, "!MD5", 4 ) && sv_send_logos.value )
	{
...
void SV_TSourceEngineQuery( netadr_t from )
{
	...
	if( Q__stricmp( GI->gamedir, "valve" ))
	{
