...
#pragma warning( disable:4133 ) //magic nipples - fixes russia
...
qboolean SV_InitPhysicsAPI( void )
{
	...
	if( pPhysIface )
	{
		if( pPhysIface( SV_PHYSICS_INTERFACE_VERSION, &gPhysicsAPI, &svgame.physFuncs ))
		{
			...
			//host.features |= ENGINE_WRITE_LARGE_COORD; //DOOM
			//host.features |= ENGINE_COMPUTE_STUDIO_LERP;
			host.features |= ENGINE_IMPROVED_LINETRACE;
			host.features |= ENGINE_QUAKE_COMPATIBLE;
