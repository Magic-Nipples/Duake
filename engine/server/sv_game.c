...
void SV_QueueChangeLevel( const char *level, const char *landname )
{
	...
	if( smooth && !Q__stricmp( sv.name, level ))
	{
...
void pfnSetModel( edict_t *e, const char *m )
{
	...
	if( COM_CheckString( name ))
	{
		...
		{
			if( !Q__stricmp( sv.model_precache[i], name ))
			{
				found = true;
				break;
			}
...
int pfnDecalIndex( const char *m )
{
	...
	{
		if( !Q__stricmp( host.draw_decals[i], m ))
			return i;
	}
...
{
	if( !Q__stricmp( "oem_end_credits", pszSection ))
		Host_Credits ();
	else Cbuf_AddText( "\ndisconnect\n" );
}
...
qboolean SV_ParseEdict( char **pfile, edict_t *ent )
{
	...
	if( !Q__stricmp( GI->gamedir, "ce" ))
	{
	 	if( !Q__stricmp( sv.name, "ce08_02" ) && !Q__stricmp( classname, "info_player_start_force" ))
			adjust_origin = true;
	}
#endif
