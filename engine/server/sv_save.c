...
static int EntryInTable( SAVERESTOREDATA *pSaveData, const char *pMapName, int index )
{
	...
	{
		if ( !Q__stricmp( pSaveData->levelList[i].mapName, pMapName ))
			return i;
	}
...
static int SaveGameSlot( const char *pSaveName, const char *pSaveComment )
{
	...
	// output to disk
	//if( !Q__stricmp( pSaveName, "quick" ) || !Q__stricmp( pSaveName, "autosave" )) //magic nipples - prevent quick save incrementing
...
static void LoadAdjacentEnts( const char *pOldLevel, const char *pLandmarkName )
{
	...
	for( i = 0; i < currentLevelData.connectionCount; i++ )
	{
		...
		if( !Q__stricmp( currentLevelData.levelList[i].mapName, pOldLevel ))
			foundprevious = true;

		for( test = 0; test < i; test++ )
		{
			// only do maps once
			if( !Q__stricmp( currentLevelData.levelList[i].mapName, currentLevelData.levelList[test].mapName ))
				break;
		}
...
qboolean SV_GetSaveComment( const char *savename, char *comment )
{
	...
	if( Q__stricmp( pFieldName, "GameHeader" ))
	{
	...
	for( i = 0; i < nNumberOfFields; i++ )
	{
		...
		if( !Q__stricmp( pFieldName, "comment" ))
		{
			Q_strncpy( description, pData, nFieldSize );
		}
		else if( !Q__stricmp( pFieldName, "mapName" ))
		{
