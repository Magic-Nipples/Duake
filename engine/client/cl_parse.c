...
void CL_ParseStaticEntity( sizebuf_t *msg )
{
	...
	{
		// auto 'solid' faces
		if( FBitSet( ent->model->flags, MODEL_TRANSPARENT ) && CL_IsQuakeCompatible( )) //DOOM - turned off
		{
...
void CL_ParseResLocation( sizebuf_t *msg )
{
	...
	if( url && ( !Q__strnicmp( "http://", url, 7 ) || !Q__strnicmp( "https://", url, 8 )))
	{
...
void CL_ParseUserMessage( sizebuf_t *msg, int svc_num )
{
	...
	if( clgame.msg[i].func )
	{
		...
		if( !Q__stricmp( clgame.msg[i].name, "HudText" ) && !Q__stricmp( GI->gamefolder, "valve" ))
		{
...
void CL_ParseServerMessage( sizebuf_t *msg, qboolean normal_message )
{
	...
	while( 1 )
	{
		...
		case svc_event:
			CL_ParseEvent( msg );
			cl.frames[cl.parsecountmod].graphdata.event += MSG_GetNumBytesRead( msg ) - (word)bufStart;
			break;
		case svc_changing:
		...
		case svc_sound:
			...
			cl.frames[cl.parsecountmod].graphdata.sound += MSG_GetNumBytesRead( msg ) - (word)bufStart;
			break;
		case svc_time:
		...
		case svc_stufftext:
			...
			if( !Q__strnicmp( s, "disconnect", 10 ) && cls.signon != SIGNONS )
				break; // too early
#endif
			Cbuf_AddText( s );
			break;
		...
		case svc_clientdata:
			...
			cl.frames[cl.parsecountmod].graphdata.client += MSG_GetNumBytesRead( msg ) - (word)bufStart;
			break;
		case svc_resource:
		...
		case svc_temp_entity:
			...
			cl.frames[cl.parsecountmod].graphdata.tentities += MSG_GetNumBytesRead( msg ) - (word)bufStart;
			break;
		case svc_setpause:
		...
		case svc_finale:
			...
			break;
		case svc_cdtrack: //magic nipples - not used by quake
		...
		case svc_packetentities:
			...
			cl.frames[cl.parsecountmod].graphdata.entities += MSG_GetNumBytesRead( msg ) - (word)bufStart - (word)playerbytes;
			break;
		...
		default:
			...
			cl.frames[cl.parsecountmod].graphdata.usr += MSG_GetNumBytesRead( msg ) - (word)bufStart;
			break;
		}
	}

	cl.frames[cl.parsecountmod].graphdata.msgbytes += MSG_GetNumBytesRead( msg ) - (word)cls.starting_count;
