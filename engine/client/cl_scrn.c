...
qboolean SCR_LoadVariableWidthFont( const char *fontname )
{
	...
	cls.creditsFont.hFontTexture = GL_LoadTexture( fontname, NULL, 0, TF_IMAGE | TF_NEAREST ); //WOLF 3D - no filtering on credits font
	R_GetTextureParms( &fontWidth, NULL, cls.creditsFont.hFontTexture );
...
void SCR_RegisterTextures(void)
{
	...
	if (FS_FileExists("gfx/paused.lmp", false))
		cls.pauseIcon = GL_LoadTexture("gfx/paused.lmp", NULL, 0, TF_IMAGE | TF_NEAREST);
	else if (FS_FileExists("gfx/pause.lmp", false))
		cls.pauseIcon = GL_LoadTexture("gfx/pause.lmp", NULL, 0, TF_IMAGE | TF_NEAREST);

	if (FS_FileExists("gfx/lambda.lmp", false))
	{
		if (cl_allow_levelshots->value)
			cls.loadingBar = GL_LoadTexture("gfx/lambda.lmp", NULL, 0, TF_IMAGE | TF_LUMINANCE);
		else cls.loadingBar = GL_LoadTexture("gfx/lambda.lmp", NULL, 0, TF_IMAGE | TF_NEAREST);
	}
	else if (FS_FileExists("gfx/loading.lmp", false))
	{
		if (cl_allow_levelshots->value)
			cls.loadingBar = GL_LoadTexture("gfx/loading.lmp", NULL, 0, TF_IMAGE | TF_LUMINANCE);
		else cls.loadingBar = GL_LoadTexture("gfx/loading.lmp", NULL, 0, TF_IMAGE | TF_NEAREST);
	}

	//cls.tileImage = GL_LoadTexture("gfx/backtile.lmp", NULL, 0, TF_NOMIPMAP);
	cls.background = GL_LoadTexture("gfx/shell/splash.bmp", NULL, 0, TF_IMAGE);
}
...
{
	Cvar_SetValue("viewsize", Q_max(scr_viewsize->value - 10, 110));//Cvar_SetValue( "viewsize", Q_max( scr_viewsize->value - 10, 30 ));
}
...
void SCR_Init( void )
{
	...
	scr_viewsize = Cvar_Get( "viewsize", "110", FCVAR_ARCHIVE, "screen size" );
