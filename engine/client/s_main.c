...
int S_GetCurrentDynamicSounds( soundlist_t *pout, int size )
{
	...
	{
		if( !channels[i].sfx || !channels[i].sfx->name[0] || !Q__stricmp( channels[i].sfx->name, "*default" ))
...
void S_Music_f( void )
{
	//DOOM //magic nipples - this fixes gamestartup.mp3 being interrupted during demo play in the menu
...
qboolean S_Init( void )
{
	...
	s_mixahead = Cvar_Get( "_snd_mixahead", "0.11", 0, "how much sound to mix ahead of time" ); //magic nipples - was 0.12
