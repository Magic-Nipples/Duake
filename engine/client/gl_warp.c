...
void R_SetupSky( const char *skyboxname )
{
	...
	if( result == SKYBOX_MISSED && Q__stricmp( loadname, DEFAULT_SKYBOX_PATH ))
	{
...
void R_InitSkyClouds( mip_t *mt, texture_t *tx, qboolean custom_palette )
{
	...
	// load it in
	tr.solidskyTexture = GL_LoadTextureInternal( "solid_sky", &r_temp, TF_NOMIPMAP | TF_SKY_NEAREST);
	...
	// load it in
	tr.alphaskyTexture = GL_LoadTextureInternal( "alpha_sky", &r_temp, TF_NOMIPMAP | TF_SKY_NEAREST);
...
void EmitWaterPolys( msurface_t *warp, qboolean reverse )
{
	...
	//lit water - only used for forcing rendering mode past usual code
	//pglBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	//pglColor4ub(255, 255, 255, 155);
	//pglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	//pglEnable(GL_BLEND);

	if (cl.local.waterlevel >= 3) //lit water - func_water has same params are yotd
	{
	...
	//pglDisable(GL_BLEND); lit water - only used for forcing rendering mode past usual code
