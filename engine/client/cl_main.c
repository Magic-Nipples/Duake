...
void CL_BeginUpload_f( void )
{
	...
	if( Q_strlen( name ) != 36 || Q__strnicmp( name, "!MD5", 4 ))
	{
...
void CL_ParseNETInfoMessage( netadr_t from, sizebuf_t *msg, const char *s )
{
	...
	if( !Q__stricmp( val, "protocol" ))
		SetBits( errorBits, NET_ERROR_PROTO_UNSUPPORTED );
	else if( !Q__stricmp( val, "undefined" ))
...
void CL_ProcessFile( qboolean successfully_received, const char *filename )
{
	...
	if( !Q__strnicmp( filename, DEFAULT_SOUNDPATH, sound_len ))
		pfilename += sound_len;

	for( p = cl.resourcesneeded.pNext; p != &cl.resourcesneeded; p = p->pNext )
	{
		if( !Q__strnicmp( filename, "!MD5", 4 ))
		{
		...
		else
		{
			if( p->type == t_generic )
			{
				if( !Q__stricmp( p->szFileName, filename ))
					break;
			}
			else
			{
				if( !Q__stricmp( p->szFileName, pfilename ))
					break;
			}
		}
	}
...
void CL_InitLocal( void )
{
	...
	// userinfo
	cl_nopred = Cvar_Get( "cl_nopred", "1", FCVAR_ARCHIVE|FCVAR_USERINFO, "disable client movement prediction" );
	...
	cl_lw = Cvar_Get( "cl_lw", "0", FCVAR_ARCHIVE|FCVAR_USERINFO, "enable client weapon predicting" );
	...
	cl_draw_particles = Cvar_Get( "r_drawparticles", "1.0", FCVAR_CHEAT, "render particles" );
	...
	Cvar_Get( "hud_scale", "1", FCVAR_ARCHIVE|FCVAR_VIDRESTART, "scale hud at current resolution" ); //FCVAR_ARCHIVE|FCVAR_LATCH
