...
{
	r_sprite_lerping = Cvar_Get( "r_sprite_lerping", "0", FCVAR_ARCHIVE, "enables sprite animation lerping" );
...
static dframetype_t *R_SpriteLoadFrame( model_t *mod, void *pin, mspriteframe_t **ppframe, int num )
{
	...
	if( FBitSet( mod->flags, MODEL_CLIENT )) // it's a HUD sprite Magic Nipples
	{
		Q_snprintf( texname, sizeof( texname ), "#HUD/%s(%s:%i%i).spr", sprite_name, group_suffix, num / 10, num % 10 );
		gl_texturenum = GL_LoadTexture( texname, pin, pinframe->width * pinframe->height * bytes, TF_IMAGE | TF_NEAREST); //DOOM - no filtering on hud sprites
	}
	else
	{
...
{
	if(e->curstate.renderfx == kRenderFxNoDissipation)//if( e->curstate.rendermode == kRenderGlow )
	{
		float	blend;
		vec3_t	v;
...
void R_DrawSpriteModel( cl_entity_t *e )
{
	...
	if( R_SpriteHasLightmap( e, psprite->texFormat ))
	{
		...
		{
			float midcol = ((color2[0] + color2[1] + color2[2]) / 3);
			VectorSet(color2, midcol, midcol, midcol);
		}
	...
	case SPR_FWD_PARALLEL_UPRIGHT:
		...
		if(e->curstate.iuser4 == 1)
			VectorSet(v_up, 0.0f, 0.0f, 1.0f);
		else
			VectorSet( v_up, 0.0f, 0.0f, 1.15f ); //DOOM - stretch model loaded sprites slightly to mimic the vanilla game | 0.0f, 0.0f, 1.0f

		VectorSet( v_right, RI.vforward[1], -RI.vforward[0], 0.0f );
		VectorNormalize( v_right );

		psprite->facecull = SPR_CULL_NONE; //draw both sides when doom prop
		break;
	...
	if( R_SpriteHasLightmap( e, psprite->texFormat ))
	{
		...
		if ((r_overbright->value > 0)) //pglTexEnvi( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
		{
