...
====================
*/
static void PIC_DrawGeneric( float x, float y, float width, float height, const wrect_t *prc, int iFlags)
{
	float	s1, s2, t1, t2;
	int	w, h;
	...
	PicAdjustSize( &x, &y, &width, &height, iFlags );
...
void pfnPIC_Draw( int x, int y, int width, int height, const wrect_t *prc )
{
	...
	PIC_DrawGeneric( x, y, width, height, prc, 0 );
}
...
/*
=========
pfnPIC_DrawMessage //QUAKE
=========
*/
void pfnPIC_DrawMessage(int x, int y, int width, int height, const wrect_t* prc)
{
	...
	GL_SetRenderMode(kRenderTransAlpha);
	PIC_DrawGeneric(x, y, width, height, prc, 1);
}
...
static int UI_DrawConsoleString( int x, int y, const char *string )
{
	...
	drawLen = Con_DrawString( x, y, string, gameui.ds.textColor ); //magic nipples - Con_DrawSecString
