...
void CL_InitCDAudio( const char *filename )
{
	...
	{
		if (FS_FileExists("media/cdaudio.txt", false)) //now load default music
			afile = FS_LoadFile("media/cdaudio.txt", NULL, false);
		else
			CL_CreatePlaylist(filename);// create a default playlist
	}
	else
	{
	...
	{
		if( !Q__stricmp( token, "blank" )) token[0] = '\0';
...
====================
*/
static void SPR_AdjustSize( float *x, float *y, float *w, float *h, int iFlags )
{
	float	xscale, yscale;
	...
	if (iFlags == 1)
	{
		xscale = glState.width / (float)clgame.scrInfo.iWeaponWidth;
		yscale = glState.height / (float)clgame.scrInfo.iWeaponHeight;
	...
	{
		if (x) *x *= xscale;
		if (y) *y *= (glState.height / (float)clgame.scrInfo.iHeight);
		if (w) *w *= xscale;
		if (h) *h *= yscale;
	}
	else
...
====================
*/
void PicAdjustSize( float *x, float *y, float *w, float *h, int iFlags )
{
	float	xscale, yscale;
	...
	if (iFlags == 1)
	{
		xscale = glState.width / (float)clgame.scrInfo.iMessageWidth;
		yscale = glState.height / (float)clgame.scrInfo.iMessageHeight;
	}
	else
	{
		xscale = glState.width / (float)clgame.scrInfo.iWidth;
...
====================
*/
static void SPR_DrawGeneric( int frame, float x, float y, float width, float height, const wrect_t *prc, int iFlags )
{
	float	s1, s2, t1, t2;
	int	texnum;
	...
	// scale for screen sizes
	SPR_AdjustSize( &x, &y, &width, &height, iFlags );
...
static void CL_DrawLoading(float percent)
{
	...
	if (cls.background && CVAR_TO_BOOL(v_dark))
	{
		pglColor4ub(255, 255, 255, 255);
		GL_SetRenderMode(kRenderNormal);
		R_DrawStretchPic(0, 0, glState.width, glState.height, 0, 0, 1, 1, cls.background);
	}
...
void CL_LinkUserMessage( char *pszName, const int svc_num, int iSize )
{
	...
	for( i = 0; i < MAX_USER_MESSAGES && clgame.msg[i].name[0]; i++ )
	{
		...
		if( !Q__stricmp( clgame.msg[i].name, pszName ))
		{
...
const model_t *CL_GetSpritePointer( HLSPRITE HLSPRITE )
{
	...
	if( HLSPRITE <= 0 || HLSPRITE >= MAX_CLIENT_SPRITES )
		return NULL; // bad image
	mod = &clgame.sprites[HLSPRITE];
...
static void pfnSPR_SetAlpha(HLSPRITE hPic, int r, int g, int b, int a)
{
	clgame.ds.pSprite = CL_GetSpritePointer(hPic);
	clgame.ds.spriteColor[0] = bound(0, r, 255);
	clgame.ds.spriteColor[1] = bound(0, g, 255);
	clgame.ds.spriteColor[2] = bound(0, b, 255);
	clgame.ds.spriteColor[3] = bound(0, a, 255);
}
...
static void pfnSPR_Draw( int frame, int x, int y, const wrect_t *prc )
{
	...
	SPR_DrawGeneric( frame, x, y, -1, -1, prc, 0 );
}
...
static void pfnSPR_DrawHolesWeapon(int frame, int x, int y, const wrect_t* prc)
{
	pglEnable(GL_ALPHA_TEST);
	pglBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	pglEnable(GL_BLEND);

	SPR_DrawGeneric(frame, x, y, -1, -1, prc, 1);
...
static client_sprite_t *pfnSPR_GetList( char *psz, int *piCount )
{
	...
	for( slot = 0; slot < MAX_CLIENT_SPRITES && pEntry->szListName[0]; slot++ )
	{
		...
		if( !Q__stricmp( pEntry->szListName, psz ))
		{
...
void CL_FillRGBA( int x, int y, int w, int h, int r, int g, int b, int a )
{
	...
	SPR_AdjustSize( (float *)&x, (float *)&y, (float *)&w, (float *)&h, 0 );
...
static int pfnGetScreenInfo( SCREENINFO *pscrinfo )
{
	...
	{	
#ifdef WOLF3DGAME
		if (glState.height <= 240)
			mscale = 1.0;
		else if (glState.height < 700)
			mscale = 0.3334f;
			...
			mscale = 0.1f;
#endif
		xscale = fabs(2400000 / glState.height) * 0.0001;
		yscale = fabs(2000000 / glState.height) * 0.0001; //doom is 320x200 so this we have slight vertical stretching

		mscale = fabs(2400000 / glState.height) * 0.0001;

		//Con_Printf("RES X = %f, RES Y = %f", xscale, yscale);

		clgame.scrInfo.iWidth = glState.width* xscale;//floor(glState.width * xscale);
		clgame.scrInfo.iHeight = glState.height * yscale;//floor(glState.height * xscale - 40);

		clgame.scrInfo.iMessageWidth = glState.width* mscale;
		clgame.scrInfo.iMessageHeight = glState.height * mscale;

#ifdef WOLF3DGAME
		clgame.scrInfo.iWeaponWidth = glState.width * mscale;
		clgame.scrInfo.iWeaponHeight = glState.height * mscale;
		clgame.scrInfo.iWeaponScale = yscale;//mscale;
	...
	else
	{
		...
		clgame.scrInfo.iMessageWidth = glState.width;
		clgame.scrInfo.iMessageHeight = glState.height;

#ifdef WOLF3DGAME
		clgame.scrInfo.iWeaponWidth = glState.width;
		clgame.scrInfo.iWeaponHeight = glState.height;
		clgame.scrInfo.iWeaponScale = 1;
#endif
		clgame.scrInfo.iFlags &= ~SCRINFO_STRETCHED;
	}
...
static void pfnPlaySoundByName( const char *szSound, float volume )
{
	...
	S_StartSound( NULL, cl.viewentity, CHAN_WEAPON, hSound, volume, ATTN_NORM, PITCH_NORM, SND_STOP_LOOPING );
}
...
client_textmessage_t *CL_TextMessageGet( const char *pName )
{
	...
	{
		if( !Q__stricmp( pName, clgame.titles[i].pName ))
...
static int pfnDrawCharacter( int x, int y, int number, int r, int g, int b )
{
	...
	if(!(r == 0 && g == 0 && b == 0))
	{
		...
		pfnPIC_DrawMessage(x, y, -1, -1, &cls.creditsFont.fontRc[number]); //pfnPIC_DrawAdditive( x, y, -1, -1, &cls.creditsFont.fontRc[number] ); //QUAKE
	}
...
static void pfnHookEvent( const char *filename, pfnEventHook pfn )
{
	...
	for( i = 0; i < MAX_EVENTS; i++ )
	{
		...
		if( !Q__stricmp( name, ev->name ) && ev->func != NULL )
		{
...
int CL_FindModelIndex( const char *m )
{
	...
	for( i = 0; i < cl.nummodels; i++ )
	{
		...
		if( !Q__stricmp( cl.models[i+1]->name, filepath ))
			return i+1;
	}
...
static cl_enginefunc_t gEngfuncs = 
{
	...
	pfnSPR_Width,
	pfnSPR_Set,
	pfnSPR_SetAlpha, //DOOM
	pfnSPR_Draw,
	pfnSPR_DrawHoles,
	pfnSPR_DrawAdditive,
	pfnSPR_DrawHolesWeapon, //WOLF
	SPR_EnableScissor,
	SPR_DisableScissor,
	pfnSPR_GetList,
	CL_FillRGBA,
...
qboolean CL_LoadProgs( const char *name )
{
	...
	CL_InitCDAudio( "media/cdaudio_custom.txt" ); //magic nipples - load custom music first
