...
int CL_FxBlend( cl_entity_t *e )
{
	...
	case kRenderFxDistort:
		...
		else 
		{
			...
			else blend = (int) ((1.0f - ( dist - 100 ) * ( 1.0f / 512.0f )) * e->curstate.renderamt ); //( 1.0f / 400.0f )
...
void CL_AddEntityEffects( cl_entity_t *ent )
{
	...
	if( FBitSet( ent->curstate.effects, EF_DIMLIGHT )) //invincible
	{
		...
		else
		{
			...
			else dl->radius = COM_RandomFloat(200, 231);
			VectorCopy( ent->origin, dl->origin );
			dl->die = cl.time + 0.001;
		}
	}	

	if( FBitSet( ent->curstate.effects, EF_BRIGHTLIGHT )) //quad damage
	{			
		dlight_t	*dl = CL_AllocDlight( ent->index );
		dl->color.r = 5;
		dl->color.g = 5;
		dl->color.b = 75; //250
		if( ent->player ) dl->radius = 400; // don't flickering
		else dl->radius = COM_RandomFloat( 200, 240 ); //plasma shot
		VectorCopy( ent->origin, dl->origin );
		dl->die = cl.time + 0.001;
		dl->origin[2] += 16.0f;
	}
	...
	// add light effect
	if( FBitSet( ent->curstate.effects, EF_LIGHT )) //bfg
	{
		dlight_t	*dl = CL_AllocDlight( ent->index );
		dl->color.r = 5;
		dl->color.g = 75;
		dl->color.b = 5; //250
		if (ent->player) dl->radius = 400; // don't flickering
		else dl->radius = COM_RandomFloat(280, 360);
		VectorCopy(ent->origin, dl->origin);
		dl->die = cl.time + 0.001;
		dl->origin[2] += 16.0f;
	}
...
int CL_DecalIndexFromName( const char *name )
{
	...
	{
		if( !Q__stricmp( name, host.draw_decals[i] ))
			return i;
	}
