...
void R_StudioInit( void )
{
	...
	r_shadow_height = Cvar_Get("r_shadow_height", "0", 0, "shadow height");
	r_shadow_x = Cvar_Get("r_shadow_x", "0", 0, "shadow distance x axis");
	r_shadow_y = Cvar_Get("r_shadow_y", "0", 0, "shadow distance y-axis");
	r_shadow_alpha = Cvar_Get("r_shadow_alpha", "0.75", 0, "shadow opacity");
	...
	r_elight_speed = Cvar_Get("r_elight_speed", "0", FCVAR_ARCHIVE, "fade speed of elights");
	r_lighting_interp = Cvar_Get("r_lightlerp_speed", "0", FCVAR_ARCHIVE, "studio lighting interpolation and speed");
...
void R_StudioMergeBones( cl_entity_t *e, model_t *m_pSubModel )
{
	...
	for( i = 0; i < m_pStudioHeader->numbones; i++ ) 
	{
		...
		{
			if( !Q__stricmp( pbones[i].name, g_studio.cached_bonenames[j] ))
			{
...
static void R_StudioDynamicLight(cl_entity_t* ent, alight_t* plight)
{
	...
	if (!CVAR_TO_BOOL(r_colorlightmap))
	{
		float midcol = ((plight->color[0] + plight->color[1] + plight->color[2]) / 3);
		VectorSet(plight->color, midcol, midcol, midcol);
	}

	VectorSet(plight->plightvec, 0.0f, 0.0f, -1.0f); //DOOM - Quake does not have angled lights
...
void R_StudioLighting( float *lv, int bone, int flags, vec3_t normal )
{
	...
	else 
	{
		...
		//	r = r_studio_lambert->value - r_studio_lambert->value;
		//else
	...
	for( i = 0; i < g_studio.numlocallights; i++ )
	{
		...
		if( r > 0.0f )
		{
			...
			if( light[i][3] == 0.0f )
			{
				...
				if (light[i][3] > 0.055f) //magic nipples - cap elight distance so it doesn't look bad.
					light[i][3] = 0.055f;
			}
...
static void R_StudioLoadTexture( model_t *mod, studiohdr_t *phdr, mstudiotexture_t *ptexture )
{
	...
	if( !Q__strnicmp( ptexture->name, "DM_Base", 7 ) || !Q__strnicmp( ptexture->name, "remap", 5 ))
	{
		int	i, size;
		char	val[6];
		byte	*pixels;
		...
		if( !Q__strnicmp( ptexture->name, "DM_Base", 7 ))
		{
