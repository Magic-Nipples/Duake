...
void V_CalcViewRect( void )
{
	...
	int sprscale = 1.0f;
	int offsetX, offsetY, offsetBottom, offsetWidth, offsetHeight = 0;

	// intermission is always full screen	
	if( cl.intermission ) size = 120.0f;
	else size = scr_viewsize->value;

	//clgame.viewport[0]		- X start point
	//clgame.viewport[1]		- Y start point
	//clgame.viewport[2]		- Width Size
	//clgame.viewport[3]		- Height Size

	if (Cvar_VariableInteger("hud_scale"))
	{
		if (glState.height <= 240)
			sprscale = 1; //no change
	...
	if (size >= 120.0f)
	{
		...
		offsetWidth = glState.width;
		offsetHeight = glState.height;
	}
	else if (size == 110.0f)
	{
		offsetX = 0;
		offsetY = 0;
#ifndef WOLF3DGAME
		offsetBottom = 34 * sprscale;
#else
		offsetBottom = 44 * sprscale; //size of hud bottom piece + scale size.
#endif

		offsetWidth = glState.width;
		offsetHeight = glState.height - offsetBottom;
	...
	else
	{
		offsetX = 8 * sprscale;
		offsetY = 2 * sprscale;
		offsetBottom = 46 * sprscale; //size of hud bottom piece + scale size.

		offsetWidth = glState.width - (offsetX * 2);
		offsetHeight = glState.height - (offsetBottom - offsetY);

		offsetHeight -= 8;
	}

		clgame.viewport[0] = floor(offsetX);
		clgame.viewport[1] = floor(offsetY);
		clgame.viewport[2] = ceil(offsetWidth);
		clgame.viewport[3] = ceil(offsetHeight);
...
void V_PostRender( void )
{
	...
	{
		//SCR_TileClear(); //DOOM - no tile texture. overlaps hud during loading bar.
