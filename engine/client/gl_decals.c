...
void DrawSingleDecal(decal_t* pDecal, msurface_t* fa)
{
	float* v;
	int	i, j, numVerts;
	int decalIndex, decalTexIndex, iFrame;
	...
	//see if any tag decals in use
	for (j = 0; j < ARRAYSIZE(tag_decals); j++)
	{
		decalIndex = CL_DecalIndexFromName(tag_decals[j]);
		decalTexIndex = CL_DecalIndex(decalIndex);

		if (pDecal->texture == decalTexIndex)
		{
			//Con_DPrintf("TAG DECAL MATCH: %s\n", tag_decals[j]);

			if (!strcmp(tag_decals[j], tag_decals[0]))
				decal_type = 1;
			else if (!strcmp(tag_decals[j], tag_decals[1]))
				decal_type = 2;
			else if (!strcmp(tag_decals[j], tag_decals[2]))
				decal_type = 3;
			else if (!strcmp(tag_decals[j], tag_decals[3]))
				decal_type = 4;
		}
	}
	...
	{
	case 1: //FADE
		iFrame = (int)(cl.time * 10) % ARRAYSIZE(shade_cycle); //10 is the fps
		decal_name = shade_cycle[iFrame];

		//need to get decal index and then offset index to texture table
		decalIndex = CL_DecalIndexFromName(decal_name);
		decalTexIndex = CL_DecalIndex(decalIndex);

		//apply texture to plane
		GL_Bind(GL_TEXTURE0, decalTexIndex);

		break;

	case 2: //FADE CORNER
		iFrame = (int)(cl.time * 10) % ARRAYSIZE(shade_cycle);
		decal_name = shade_cycle[iFrame];

		if (strcmp(decal_name, "{shadenone"))
			sprintf(copyname, "%sc", decal_name);
		else
			strcpy(copyname, decal_name);

		decalIndex = CL_DecalIndexFromName(copyname);
		decalTexIndex = CL_DecalIndex(decalIndex);
	...
	case 3: //FLICKER
		iFrame = (int)(cl.time * 8) % ARRAYSIZE(shade_flicker);
		decal_name = shade_flicker[iFrame];
...
{	
	...
	{
		//WOLF 3D - shade decals draw here:
		for( p = fa->pdecals; p; p = p->pnext )
		{
