...
void GL_BuildLightmapWater(model_t* mod, msurface_t* fa)
{
	...
	for (poly = fa->polys; poly != NULL; poly = poly->next)
	{
		...
		for (i = 0; i < lnumverts; i++)
		{
			...
			// lightmap texture coordinates
			s = DotProduct(vec, info->lmvecs[0]) + info->lmvecs[0][3] - info->lightmapmins[0];
			s += fa->light_s * sample_size;
			s += sample_size * 0.5f;
			s /= BLOCK_SIZE * sample_size; //fa->texinfo->texture->width;

			t = DotProduct(vec, info->lmvecs[1]) + info->lmvecs[1][3] - info->lightmapmins[1];
			t += fa->light_t * sample_size;
			t += sample_size * 0.5f;
			t /= BLOCK_SIZE * sample_size; //fa->texinfo->texture->height;
...
static void R_BuildLightMap( msurface_t *surf, byte *dest, int stride, qboolean dynamic )
{
	...
	for( t = 0; t < tmax; t++, dest += stride )
	{
		for( s = 0; s < smax; s++ )
		{
			...
			if (!CVAR_TO_BOOL(r_colorlightmap)) //is this a good spot for this?
			{
				//float midcol = dest[0] * 0.21 + dest[1] * 0.71 + dest[2] * 0.07;//((dest[0] + dest[1] + dest[2]) / 3);
				//dest[0] = midcol;
				//dest[1] = midcol;
				//dest[2] = midcol;
				vec3_t midcol;
				...
				lum[1] = 0.7152;
				lum[2] = 0.0722;
				midcol[0] = pow(dest[0], 2.15f);
				midcol[1] = pow(dest[1], 2.15f);
				midcol[2] = pow(dest[2], 2.15f);

				float gray = DotProduct(midcol, lum);
...
void R_BlendLightmaps( void )
{
	...
	if( RI.currententity )
	{
		...
		if ((!CVAR_TO_BOOL(r_litwater)) || (glConfig.hardware_type == GLHW_RADEON || glConfig.hardware_type == GLHW_INTEL))
		{
			switch (RI.currententity->curstate.rendermode) // check for rendermode //lit water
			{
	...
	for( i = 0; i < MAX_LIGHTMAPS; i++ )
	{
		if( gl_lms.lightmap_surfaces[i] )
		{
			...
			{
				if (surf->polys) DrawGLPolyChain(surf->polys, 0.0f, 0.0f, surf->flags);
			}
		}
	}
	...
	if( CVAR_TO_BOOL( r_dynamic ))
	{
			...
			else
			{
				...
				for( drawsurf = newsurf; drawsurf != surf; drawsurf = drawsurf->info->lightmapchain )
				{
					if( drawsurf->polys )
					{
						...
						(drawsurf->light_t - drawsurf->info->dlight_t) * (1.0f / (float)BLOCK_SIZE), drawsurf->flags);
					}
				}
		...
		for( surf = newsurf; surf != NULL; surf = surf->info->lightmapchain )
		{
			if( surf->polys )
			{
				...
				(surf->light_t - surf->info->dlight_t) * (1.0f / (float)BLOCK_SIZE), surf->flags);
			}
		}
	}
...
void R_DrawTextureChains( void )
{
	...
	for( i = 0; i < cl.worldmodel->numtextures; i++ )
	{
		...
		/*if (CL_IsQuakeCompatible() && FBitSet(s->flags, SURF_TRANSPARENT)) //DOOM - turned off
		{
...
void R_SetRenderMode( cl_entity_t *e )
{
	...
	case kRenderTransAdd:
		...
		pglBlendFunc( GL_ONE, GL_ONE );
		//pglDepthMask( GL_FALSE ); //lit water
		pglEnable( GL_BLEND );
		break;
	case kRenderTransAlpha:
		pglEnable( GL_ALPHA_TEST );
		pglTexEnvf( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
		/*if (CL_IsQuakeCompatible()) //DOOM - turned off
		{
			pglBlendFunc( GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA );
...
void R_DrawBrushModel( cl_entity_t *e )
{
	...
	//if( CL_IsQuakeCompatible() && FBitSet( clmodel->flags, MODEL_TRANSPARENT )) //DOOM - turned off
	...
	for( i = 0; i < clmodel->nummodelsurfaces; i++, psurf++ )
	{
		...
		if (FBitSet(psurf->flags, SURF_DRAWTURB) && (!FBitSet(e->curstate.effects, EF_WATERSIDES)))
		{
			//if( psurf->plane->type != PLANE_Z && !FBitSet( e->curstate.effects, EF_WATERSIDES ))
			if (psurf->plane->type != PLANE_Z)
				continue;
