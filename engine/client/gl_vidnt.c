...
void GL_CheckExtension( const char *name, const dllfunc_t *funcs, const char *cvarname, int r_ext )
{
	...
	if( cvarname )	
		parm = Cvar_Get( cvarname, "1", FCVAR_GLCONFIG, va( CVAR_GLCONFIG_DESCRIPTION, name )); // system config disable extensions
...
void VID_StartupGamma( void )
{
	...
	ClearBits(r_colorlightmap->flags, FCVAR_CHANGED);
}
...
{
	int		x = 0, y = 0, w, h;
	int		stylebits = WINDOW_STYLE| WS_MINIMIZEBOX| WS_MAXIMIZE; //magic nipples - added minimize button to this
	...
	wc.style = CS_OWNDC;// | CS_NOCLOSE; magic nipples - let users close window from button in window bar
...
void GL_InitCommands( void )
{
	...
	r_detailtextures = Cvar_Get( "r_detailtextures", "0", 0, "enable detail textures support, use '2' for autogenerate detail.txt" );
	...
	gl_wgl_msaa_samples = Cvar_Get( "gl_wgl_msaa_samples", "0", FCVAR_GLCONFIG, "enable multisample anti-aliasing" ); //4
	gl_texture_nearest = Cvar_Get( "gl_texture_nearest", "1", FCVAR_ARCHIVE, "disable texture filter" );
	...
	gl_texture_lodbias =  Cvar_Get( "gl_texture_lod", "0.0", FCVAR_ARCHIVE, "LOD bias for mipmapped textures (perfomance|quality)" );
	...
	gl_msaa = Cvar_Get( "gl_msaa", "0", FCVAR_ARCHIVE, "enable multi sample anti-aliasing" );
	...
	r_colorlightmap = Cvar_Get("r_color_lightmap", "1", FCVAR_ARCHIVE, "enable colored lightmaps");
	r_litwater = Cvar_Get("r_litwater", "1", FCVAR_ARCHIVE, "enable lightmaps on water (broken on radeon as of now)");
	r_smear = Cvar_Get("v_melt", "0", 0, "doom melt effect");

	//magic nipples - glsl
	r_glsl = Cvar_Get("r_glsl", "1", FCVAR_ARCHIVE, "Render glsl scripts");
