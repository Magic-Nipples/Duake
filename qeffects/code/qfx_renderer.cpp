...
#include "glsl_dither.h"
#include "glsl_n64point.h"
#include "glsl_palette_quake.h"
#include "glsl_palette_doom.h"
...
QFXRenderer :: QFXRenderer()
{
	...
	m_varZTrickFix = QFXSettings::Instance().GetInteger( "ZTrickFix", 0 ); //Magic Nipples - breaks quake remake...
	m_varViewportFix = QFXSettings::Instance().GetInteger( "ViewportFix", 0 );
	m_varRenderFrame = QFXSettings::Instance().GetInteger( "RenderFrame", 2 );
	m_varAnisotropy = QFXSettings::Instance().GetInteger( "Anisotropy", 0 );
	m_varMultisample = QFXSettings::Instance().GetInteger( "Multisample", 0 );
	m_varMultisampleHint = QFXSettings::Instance().GetInteger( "MultisampleHint", 0 );
	m_varBloomEnable = QFXSettings::Instance().GetInteger( "Bloom", 0 ); //1
	m_varBloomNumSteps = std::min( 16, QFXSettings::Instance().GetInteger( "BloomSteps", 0 ) ); //8
	m_varBloomDarkenPower = QFXSettings::Instance().GetFloat( "BloomDarken", 0 ); //3
	m_varBloomCombineScale = QFXSettings::Instance().GetFloat( "BloomScale", 0.0f ); //1.5
	m_varSSAOEnable = QFXSettings::Instance().GetInteger( "SSAO", 0 );
	m_varSSAOScale = QFXSettings::Instance().GetFloat( "SSAOScale", 0.0f );
	m_varSSAOMinZ = QFXSettings::Instance().GetFloat( "SSAOMinZ", 0.000f );
	m_varSSAOMaxZ = QFXSettings::Instance().GetFloat( "SSAOMaxZ", 0.0f );
	...
	m_varEmboss = QFXSettings::Instance().GetInteger( "Emboss", 0 );
	m_varEmbossScale = QFXSettings::Instance().GetFloat( "EmbossScale", 0.0f ); //0.5f
	m_varDOF = QFXSettings::Instance().GetInteger( "DOF", 0 );
	m_varDOFSpeed = QFXSettings::Instance().GetFloat( "DOFSpeed", 0.0f ); //1.0f
	m_varDOFScale = QFXSettings::Instance().GetFloat( "DOFScale", 0.0f ); //2.0f
	m_varDOFMinZ = QFXSettings::Instance().GetFloat( "DOFMinZ", 0.000f ); //0.005f
	m_varVignette = QFXSettings::Instance().GetInteger( "Vignette", 0 ); //1
	m_varVignetteScale = QFXSettings::Instance().GetFloat( "VignetteScale", 0.0f ); //1.25f
	m_varVignetteScale = std::min( 2.0f, std::max( 0.0f, m_varVignetteScale ) );

	//m_varPixelate = QFXSettings::Instance().GetInteger("PixelateResolution", 0);
	//m_varDoom = QFXSettings::Instance().GetInteger("PaletteLimit", 0);
	//m_varDither = QFXSettings::Instance().GetInteger("Dither", 0);
	//m_varN64 = QFXSettings::Instance().GetInteger("3PointFilter", 0);
}
...
void QFXRenderer :: CheckHardware()
{
	...
	if (m_varPixelate > 0)
		QFXLog::Instance().Printf("Pixelate Resolution: %i\n", m_varPixelate);
	//========================================================================================
	m_varDoom = QFXSettings::Instance().GetInteger("PaletteLimit", 0);

	if (m_varDoom > 0)
		QFXLog::Instance().Printf("Palette Limit: ON\n");
	//========================================================================================
	m_varDither = QFXSettings::Instance().GetInteger("Dither", 0);

	if (m_varDither == 1)
		QFXLog::Instance().Printf("Dithering: ON\n");
	else if (m_varDither >= 2)
		QFXLog::Instance().Printf("Dithering after pixelate and palette limit: ON\n");
	//========================================================================================
	m_var3point = QFXSettings::Instance().GetInteger("3PointFilter", 0);

	if (m_var3point > 0)
		QFXLog::Instance().Printf("3-point filtering: %i\n", m_var3point);
	//========================================================================================
}
...
void QFXRenderer :: InitializeGL()
{
	...
	m_pPixelate = new QFXShader(g_szVertexShader_Pixelate, g_szFragmentShader_Pixelate);
	m_pDOOM = new QFXShader(g_szVertexShader_DOOM, g_szFragmentShader_DOOM);
	m_pQUAKE = new QFXShader(g_szVertexShader_QUAKE, g_szFragmentShader_QUAKE);
	m_pDither = new QFXShader(g_szVertexShader_Dither, g_szFragmentShader_Dither);
	m_pN64 = new QFXShader(g_szVertexShader_n64point, g_szFragmentShader_n64point);
}
...
void QFXRenderer::PostProcess_Pixelate()
{
	const int screenWidth = m_3DViewport[2];
	const int screenHeight = m_3DViewport[3];
	...
	// perform correction
	m_pPixelate->Bind();
	m_pPixelate->SetParameter4f(0, (float)screenWidth, (float)screenHeight, (float)m_varPixelate, 0);
	RenderFSQ(screenWidth, screenHeight);
...
void QFXRenderer::PostProcess_Doom()
{
	const int screenWidth = m_3DViewport[2];
	const int screenHeight = m_3DViewport[3];
...
void QFXRenderer::PostProcess_Dither()
{
	const int screenWidth = m_3DViewport[2];
	const int screenHeight = m_3DViewport[3];
...
void QFXRenderer::PostProcess_n64()
{
	const int screenWidth = m_3DViewport[2];
	const int screenHeight = m_3DViewport[3];
	...
	// perform correction
	m_pN64->Bind();
	m_pN64->SetParameter4f(0, (float)screenWidth, (float)screenHeight, (float)m_var3point, 0);
	RenderFSQ(screenWidth, screenHeight);
