...
QFXSettings :: QFXSettings()
{
	...
	if (!GetModuleFileName( nullptr, m_szExeName, sizeof(m_szExeName)-1 )) {
		m_szExeName[0] = 0;
	}
	...
	if ( m_szExeName[1] != ':' ) {
		...
		strcat_s( m_szIniFile, "\\" QFX_GAME_DIRECTORY "\\" "shaders.cfg" ); //QFX_LIBRARY_TITLE ".ini"
...
int QFXSettings :: GetInteger( const char *setting, int defaultValue )
{
	...
	if ( value == -99999 )
		return GetPrivateProfileInt( "Default", setting, defaultValue, m_szIniFile );
	else
		return value;
}
...
{
	char buffer[32];
	GetPrivateProfileString( m_szExeName, setting, nullptr, buffer, sizeof(buffer)-1, m_szIniFile );
	if ( !strlen( buffer ) ) {
		GetPrivateProfileString( "Default", setting, nullptr, buffer, sizeof(buffer)-1, m_szIniFile );
		if ( !strlen( buffer ) )
			return defaultValue;
	}
...
bool QFXSettings :: CheckEXE( const char *s ) const
{
	return !_stricmp( m_szExeName, s );
}
