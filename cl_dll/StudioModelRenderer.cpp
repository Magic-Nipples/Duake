...
void CStudioModelRenderer::Init( void )
{
	...
	m_pCvarInterpolateModels = CVAR_CREATE("r_lerphlmodels", "1", FCVAR_ARCHIVE);
}
...
// TODO: could cache lazily, stored in the entity
	...
	if (m_pCurrentEntity->curstate.movetype == MOVETYPE_STEP) 
	{
		...
		if (m_pCvarInterpolateModels->value > 0)
		{
			if (m_fDoInterp)
				f = f - 1.0; // ugly hack to interpolate angle, position. current is reached 0.1 seconds after being set
			else
				f = 0;
		}
		else
...
//			Con_DPrintf("%.0f %.0f\n",m_pCurrentEntity->msg_angles[0][YAW], m_pCurrentEntity->msg_angles[1][YAW] );
	...
	if (m_pRenderModel->flags & 8)	// EF_ROTATE
		angles[YAW] = anglemod(100 * m_clTime);
	...
	if( m_pCurrentEntity->curstate.renderfx == kRenderLavaDeform )
	{
		vec3_t	vScale, pScale;

		VectorCopy( m_pCurrentEntity->curstate.vuser1, pScale );

		// check for axial planes
		if( pScale[0] == 1.0f ) pScale[0] = -1.0f;
		if( pScale[1] == 1.0f ) pScale[1] = -1.0f;
		if( pScale[2] == 1.0f ) pScale[2] = -1.0f;

		if( pScale[0] == 0.0f ) pScale[0] = 1.0f;
		...
		for( i = 0; i < 3; i++)
		{
			(*m_protationmatrix)[i][0] *= vScale[i];
			(*m_protationmatrix)[i][1] *= vScale[i];
			(*m_protationmatrix)[i][2] *= vScale[i];
		}
#if 0
		float amt = 255 - ( m_clTime - m_pCurrentEntity->curstate.animtime) * 255.0;
		m_pCurrentEntity->curstate.rendermode = kRenderTransTexture;
		m_pCurrentEntity->curstate.renderamt = amt;
#endif
	}
}
...
int CStudioModelRenderer::StudioDrawModel( int flags )
{
	...
	if (flags & STUDIO_RENDER)
	{
		...
		{
			lighting.ambientlight = 256;
			lighting.shadelight = 256; 
		...
		if (IsPlayerViewModel()) //DOOM - grab color and brightness data from here since view models have all the data we need.
		{
			gHUD.m_vGunCol = lighting.color;
			gHUD.m_iGunIllum = (lighting.ambientlight + lighting.shadelight) * 1.2;

			if (gHUD.m_iGunIllum > 255)
				gHUD.m_iGunIllum = 255;

			//gEngfuncs.Con_Printf("%0.2f | %0.2f | %0.2f | %i\n", gHUD.m_vGunCol[0], gHUD.m_vGunCol[1], gHUD.m_vGunCol[2], gHUD.m_iGunIllum);
		}
		...
		entity_state_t *pentity = &m_pCurrentEntity->curstate;

		if (pentity->weaponmodel)
		{
			mstudioseqdesc_t *pseqdesc = (mstudioseqdesc_t *)((byte *)m_pStudioHeader + m_pStudioHeader->seqindex) + pentity->sequence;

			cl_entity_t saveent = *m_pCurrentEntity;

			model_t *pweaponmodel = IEngineStudio.GetModelByIndex( pentity->weaponmodel );
...
int CStudioModelRenderer::StudioDrawPlayer( int flags, entity_state_t *pplayer )
{
	...
	m_pRenderModel = m_pCurrentEntity->model;	// don't allow change playermodels!
...
====================
*/
int CStudioModelRenderer::IsPlayerViewModel( void )
{
	if( m_pCurrentEntity != gEngfuncs.GetViewModel())
		return false;

	if( m_pRenderModel != gEngfuncs.GetViewModel()->model )
		return false;
...
void CStudioModelRenderer::StudioRenderModel( void )
{
	...
	if( IsPlayerViewModel( ))
	{
		// NOTE: predicting is disabled so we need to copy render settings
		// into viewmodel here
		...
		m_pCurrentEntity->curstate.rendercolor.r = player->curstate.rendercolor.r;
		m_pCurrentEntity->curstate.rendercolor.g = player->curstate.rendercolor.g;
		m_pCurrentEntity->curstate.rendercolor.b = player->curstate.rendercolor.b;
...
void CStudioModelRenderer::StudioRenderFinal_Hardware( void )
{
	...
	if( gHUD.ClientInvisible( ) && IsPlayerViewModel( ) && !IEngineStudio.GetForceFaceFlags())
	{
		rendermode = kRenderTransTexture;
		m_pCurrentEntity->curstate.renderamt = 60;
	}
