...
void IN_MouseMove ( float frametime, usercmd_t *cmd)
{
	...
	if ( !iMouseInUse && !g_iVisibleMouse && !gHUD.m_iIntermission )
	{
		...
		if (CVAR_GET_FLOAT("sv_viewlag") > 0)
		{
			gHUD.m_fMouseX = mouse_x;
			if (lookupdown->value)
				gHUD.m_fMouseY = mouse_y;
			else
				gHUD.m_fMouseY = 0;
		}
		else
		{
			gHUD.m_fMouseX = gHUD.m_fMouseY = 0;
		}
...
void IN_Init (void)
{
	...
	lookupdown				= gEngfuncs.pfnRegisterVariable("m_updown", "1", FCVAR_ARCHIVE);
